...
PROJECT=seafile-server-urup
DESCRIPTION='SeaFile Server UnRooted UnProxxed - A container which serves a SeaFile server modified for usage with a non-root web path and an external proxy'
SEAFILE_VERSION=9.0.2
# BE AWARE: focal-1.1.0 is not an official release! See https://github.com/phusion/baseimage-docker/issues/602
PHUSION_VERSION=focal-1.0.0
IMAGE_BUILD=phusion1
...
tar -xvf generated/seafile-server_${SEAFILE_VERSION}_x86-64.tar.gz -C generated/
echook
...
sed -e "s/PHNVERARG/${PHUSION_VERSION}/g" -e "s/SFLVERARG/${SEAFILE_VERSION}/g" -e "s/SFLDESCARG/${DESCRIPTION}/g" Dockerfile.tpl > Dockerfile
echook

FULLPATH=${PROJECT}:${SEAFILE_VERSION}-${IMAGE_BUILD}
